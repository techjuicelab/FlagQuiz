...
생성물 여백이 좁으면 `node scripts/prepare-images.mjs --only <id> --inset .12`로 원본 전체를 축소하고 사방에 화풍별 배경을 더할 수 있다. 기본값은 `0`이며, `0` 이상 `0.4` 미만만 허용한다. 양수일 때는 원본을 자르지 않고 내부 상자에 담아 최종 A 768×576 / B 1024×768 캔버스로 만든다. 변환 보고서의 `inset` 값을 제작 기록에 보존한다. 이 옵션은 원장 승인 상태를 바꾸지 않는다.

원본 배경이 지정 색과 다르면 새 여백과의 경계가 보일 수 있으므로 최종 그림을 직접 확인한다. 여백 보정은 잘못된 개수·형태를 고치지 않는다.
