...
- 링 버리기 규칙: bbox 의 가로·세로가 **둘 다** tol 보다 작은 링은 버린다(자잘한 무인도). 단 **194개국 각각의 최대 면적 링은 예외 없이 보존한다**. 보존 대상 링이 DP 후 4점 미만이 되면 그 링의 bbox 사각형(5점)으로 대체한다.
...
- [ ] 194개국 각각에 대해 최소 한 개의 링이 출력에 들어간다. 싱가포르·모나코·바티칸·투발루를 스크립트가 명시적으로 점검하고 누락 시 exit 1.
...
- mapCoords 의 모양: `{"af":[66.00,34.00],"al":[20.07,41.14], …}` — 키는 소문자 2자 code, 값은 `[lng, lat]` 배열(GeoJSON 과 같은 순서). NE 의 `LABEL_X`/`LABEL_Y`(사람이 손으로 보정한 라벨용 좌표)를 그대로 쓰고 **소수 2자리**로 반올림한다(D3 항목 1). 한 줄에 한 나라씩, code 사전순 정렬 — 나중에 한 나라 좌표를 손보정할 때 diff 가 한 줄로 남아야 한다.
...
- LABEL_X/LABEL_Y 검증: 각 나라의 라벨 점이 **그 나라 최대 면적 링의 bbox 를 2° 넓힌 범위** 안에 있어야 한다. 벗어나면 build-map.mjs 가 그 code 를 나열하며 exit 1. **전체 feature 의 bbox 로 검사하지 마라** — 프랑스(기아나)·미국(괌)·네덜란드(카리브)는 bbox 가 지구를 가로질러 아무것도 못 거른다. 스크립트에 `LABEL_OVERRIDES` 상수를 두고, 손보정한 나라는 사유 주석과 함께 적는다.
...
- [ ] 손으로 고른 표본 5개가 상식적이다: kr ≈ [127.8, 36.4], au ≈ [134.0, -24.1], br ≈ [-53.1, -10.8], va 가 로마 안쪽(≈ [12.45, 41.9]), ru 가 서쪽 본토(경도 30~100 사이, 동시베리가 아니다).
- [ ] LABEL_OVERRIDES 에서 임의의 한 줄을 지우거나 이상한 값으로 바꾸면 bbox 검증이 exit 1 한다.
- [ ] data/map-shapes.js 안에 2자 나라 코드나 ADMIN 이름 문자열이 없다(헤더 주석 제외).
- [ ] 두 파일 모두 출처·버전·tol·재생성 명령·'손으로 고치지 마세요' 헤더 주석을 갖는다.

**테스트**

- T6 의 '지도 자료' group 이 위 수용 기준 중 키 집합·값 범위·소수 자릿수·mapLand 형식을 자동으로 단언한다.
